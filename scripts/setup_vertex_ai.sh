#!/bin/bash
# AEGIS Security Co-Pilot - Vertex AI Setup Script
# Prepare environment for Vertex AI Agent Engine deployment

set -e  # Exit on any error

echo "🛡️ AEGIS Security Co-Pilot - Vertex AI Setup"
echo "=============================================="
...
check_python() {
    ...
    # # Check if Python version is compatible with Agent Engine (>=3.9, <=3.12)
    # if python3 -c "import sys; exit(0 if 3.9 <= float(f'{sys.version_info.major}.{sys.version_info.minor}') <= 3.12 else 1)"; then
    #     print_success "Python version is compatible with Vertex AI Agent Engine"
    # else
    #     print_error "Python version $PYTHON_VERSION is not compatible with Vertex AI Agent Engine"
    #     print_error "Please use Python 3.9 to 3.12"
    #     exit 1
    # fi
}
...
check_gcloud() {
    ...
    # Try to update gcloud, but don't fail if it's managed by apt
    print_status "Checking for gcloud updates..."
    if gcloud components update --quiet 2>/dev/null; then
        print_success "Google Cloud CLI updated successfully"
    else
        print_warning "gcloud components update failed - likely installed via apt"
        print_status "Trying apt update instead..."
        
        if command -v apt-get &> /dev/null; then
            sudo apt-get update && sudo apt-get --only-upgrade install google-cloud-cli -y 2>/dev/null || {
                print_warning "apt update failed or not available, continuing anyway..."
            }
        else
            print_warning "apt not available, skipping gcloud update"
        fi
        
        print_success "Continuing with existing gcloud installation"
    fi
}
...
EOF
    ...
    print_status "source .env.vertex_ai"
}

# Main setup function
main() {
    echo "Setting up AEGIS for Vertex AI Agent Engine deployment..."
    echo ""
    ...
    print_warning "Note: Make sure to source .env.vertex_ai before running the deployment script!"
}
